...
function postBuildCommand (element) {
  ...
  if (R.includes(options.recurse, keys)) {
    element[options.recurse] = R.split(',', element[options.recurse]);
  }
...
function normaliseChildren (command) {
  ...
  let normalisedCommand = R.omit([defaultSpec.labels.descendants], command);

  let children = command[defaultSpec.labels.descendants];
  ...
  //
  normalisedCommand[defaultSpec.labels.descendants] = adaptedChildren;
  ...
  if (R.is(Object, R.prop(defaultSpec.labels.descendants)(command))) {
    ...
    if (R.where({ 'Arguments': R.is(Array) }, children)) {
      ...
      if (joined.length === R.keys(resolved).length) {
        command = R.assocPath([defaultSpec.labels.descendants, 'Arguments'], resolved, command);
      } else {
      ...
      throw new Error(`command-builder.resolveArguments: "Arguments" is missing from "${defaultSpec.labels.descendants}" of command: "${command.name}"`);
    }
  } else {
    throw new Error(`command-builder.resolveArguments: "${defaultSpec.labels.descendants}" is missing from command: "${command.name}"`);
  }
