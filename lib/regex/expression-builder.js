 ...
 * @param {callback(el:string)} getOptions: (@callback) a function that returns the element builder
 ...
 *{
   ...
   an array.
 */
function buildExpressions (parentNode, getOptions) {
  const expressionsOptions = getOptions('Expressions');
  if (!expressionsOptions) {
    throw new Error('Couldn\'t get options for "Expressions" element');
  }
  const { id = '' } = expressionsOptions;
  validateId(parentNode, ['Expression', 'Expressions'], getOptions);
  ...
  return normalise(expressionGroups, getOptions);
}
...
function normalise (expressionGroups, getOptions) {
  ...
  const result = R.reduce((acc, groupName) => {
    const group = expressionGroups[groupName];
    const children = R.prop('_children')(group); // Array of "Expression"s for the group
    const groupById = R.groupBy(R.prop(expressionId))(children);

    // expressions here is a result of converting the single element arrays (created by groupBy)
    // into the item itself, ie [a] becomes a;
    //
    const expressions = R.map((item) => {
      if (item.length > 1) {
        throw new Error(`Expression(@${expressionId}): ${JSON.stringify(R.head(item)[expressionId])} already defined`);
      }
      return R.head(item);
    })(groupById);
    const expressionNames = R.keys(expressions);
    const accumulatedNames = R.keys(acc);
    const alreadyDefined = R.intersection(expressionNames, accumulatedNames);
