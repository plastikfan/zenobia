...
function buildExpressions (parentNode) {
  const expressionsOptions = getExpressionOptions('Expressions');
  if (!expressionsOptions) {
    throw new Error('Couldn\'t get options for "Expressions" element');
  }
  const { id = '' } = expressionsOptions;

  validateId(parentNode, ['Expression', 'Expressions'], getExpressionOptions);
  ...
  return normalise(expressionGroups, getExpressionOptions);
}
