...
import * as Helpers from '../../helpers/test-helpers';
...
describe('command-builder: buildNamedCommand (single)', () => {
  context('given: a command requested', () => {
    ...
    it('should: build a single command', () => {
      ...
      const commandsNode = xpath.select('/Application/Cli/Commands', document, true);
    ...
    if (commandsNode instanceof Node) {
      ...
      context('given: a normalised command', () => {
        ...
        it('should: return the correct number of ArgumentGroups\'s', () => {
          ...
          const argGroupsLength = R.keys(argumentGroups).length; // TODO: check this const argGroupsLength = argumentGroups.length

          expect(argGroupsLength).to.be.equal(4);
        });
      });
    } else {
      assert(assert.fail('Couldn\'t get Commands node.'));
    }
  });
