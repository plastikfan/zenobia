    ...
    tests.forEach((t: IUnitTestInfo) => {
      context(`given: ${t.given}`, () => {
        it('should: evaluate regular expression text successfully', () => {
          ...
          const applicationNode: xpath.SelectedValue = xpath.select('/Application', document, true);
    ...
    tests.forEach((t: IUnitTestInfo) => {
      context(`given: ${t.given}`, () => {
        it('should: throw', () => {
          ...
          if (applicationNode instanceof Node) {
            const expressions = Builder.buildExpressions(applicationNode);
            const getEvaluateOptions = (el: string): any => {
              return {
                id: 'name'
              };
            };
            ...
            expect(() => {
              const options: any = R.has('options', t) ? t.options : getEvaluateOptions('Expression');
